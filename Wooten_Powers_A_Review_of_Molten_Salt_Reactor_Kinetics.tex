...
\subsubsection{comparisons of point reactor kinetics methods}
\label{sssec:prk_comp}
With so many variations on the point reactor kinetics approach, and with such
variation in the degree of detail, the question must be asked, 
fuel salt from the start of the transient re-enters the core. This same
distinction can be seen again in figure \ref{fig:lucotte_ipk} in which the
CPK scheme matches much more closely with the IPK scheme at a lower power though
the IPK scheme does show the oscilations typical of CFR transients very clearly
in both transient evolutions which the CPK scheme misses entirely.
\par A point made in \cite{dulla_models_2005} is well evidenced in figures
...
figure \ref{reneiski_msre_ps},  while promising, at
...
also employed in the same work. In figure \ref{dulla_4x_qs} the same factor
...
a time of higher reactor power. On the other hand the PM0 model has almost
opposite behavior which can largely be attributed to its having the
highest fraction of delayed neutrons out of all the models. The larger
$\beta_{eff}$ in the PM0 model leads to increased reactivity introduction
in the begining of the transient as DNPs built up during periods of higher
reactor power decay. NEED AN EXPLANATION FOR FINAL TEMPERATURES. 
\par While PM0 and PM1 bound the other two methods a compelling takeaway from
...
the same and with DPK splitting the differnce in final averaged fuel salt
temperature between SM and the PM methods almost exactly in half. In a similar
story  
A point made in other works, \cite{zanetti_development_2016} for example, is
that accounting for fluid flow effects on DNP distributions matters more at
zero power than at power for transient simulation. This is clearly evidenced by
figures \ref{fig:shi_msre_ps} and \ref{fig:guerrieri_msfr_ulof}. In figure
\ref{fig:shi_msre_ps} a DNP model is used to simulate the MSRE pump start up
experiment and while the overall trend of the transient is captured well
the peak is underestimated and the oscilations are largely missed. Whereas in
figure \ref{fig:guerrieri_msfr_ulof} where a DNP scheme was used as well,
 comparing it with figure 
\ref{fig:fiorina_msfr_ulof} in which an MGD scheme was used, both agree on
the overall evolution of the transient, in which each used a different time
constant for the coast down of the pumps, with both showing a rapid drop off
in the reactor power accompanied by a small power peak when the DNP rich
