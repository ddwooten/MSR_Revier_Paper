...
it is found that the movement of delayed neutron precurssors ( DNPs ) through
...
displacement is particuarly challenging for PRK based approaches
...
\begin{figure}[H]
   ...
   \caption{Figure 16.c from \cite{zhang_development_2009-1} showing simulated DNP concentrations
               ...
               Axial
               values are taken at the center of the core. $C_{n}$ are DNP
               group concentrations. No units for conentration were given.
...
that the multipilication factor of a given CFR would continue to decrease
...
predictions of more rigorous models for transients occuring at higher
...
notable aspect of Figure \ref{fig:shi_msre_ps} is how the model understimated
the reactivity peak and the trailing reactivity tail but captured the
overall behavior of the transient. This behavior is consistant with the
...
necessarily indicate, however, that IPK is illsuited for thermal CFR
analysis.
...
the reactivity oscilations that would be expected of such a transient while the
...
oftraditional PRK
based approches. First how they fail to capture DNP circulation effects and secondly
...
traditional PRK based approaches are inaduquate for the task. If overall behavior
and endstates are required, PRK based approaches may be entirely adequate.
...
to an MGD solution. The most noticable features of this figure are how poorly the
DPK approaches capture the transient and how even the reformulated MPK approach
fails to capture the reactivity oscilations from DNP circulation. That said, the
...
arrising from a change in flow rate. The updated $\beta$ values in the DPK equations
...
necessarily indicate, however, that MPK based approaches are illsuited for fast
CFR analysis.
...
capturing shape or flow-dependent transients and transients occuring
at low power,
...
simply the result of computational error limitations is uknown. What is known,
...
user-input. This, in turn, makes MGD based approaches illsuited to tasks
...
of a parametic nature. This will allow for
 future
...
stepping, can be used for modelling while carraying with them the added benefit
of reduced computational cost. In this way parametric analysis of reactor operational
and safety characteriscts can be carried out in a resonable timeframe.
This, however, would require the existance of a
...
be well documented, be verrified against existing MGD based approaches, and
