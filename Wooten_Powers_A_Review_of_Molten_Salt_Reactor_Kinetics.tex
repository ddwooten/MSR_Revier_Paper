...
The key for edit remarks and content highlights. The latex structure used for highlighting text can not wrap text so in several places rather than highlight all the pertinent text a highlight sentence describing the area is given. This is also why the lines that are highlighted bleed over the edge.
\begin{enumerate}
\item{\textcolor{yellow}{Recommendations of models for transients. Suggested a
dditions are in yellow while} \colorbox{yellow}{text that I thought addresses
this question is highlighted in yellow}}
\item{\textcolor{green}{Remarks on the importance of DNPs are suggested in
green while} \\ \colorbox{green}{text that I thought addresses
this question is highlighted in green}}
\item{\textcolor{blue}{Remarks on the key challenges are suggested in
blue while} \\ \colorbox{blue}{text that I thought addresses
this question is highlited in blue}}
\item{\textcolor{red}{Remarks on the top codes are suggested in
red while} \\ \colorbox{red}{text that I thought addresses
this question is highlited in red}}
\item{\textcolor{orange}{Remarks on the best approaches are suggested in
orange while}\\  \colorbox{orange}{text that I thought addresses
this question is highlited in orange}}
\item{\textcolor{pink}{Remarks on the pros and cons of PRK approaches are 
suggested in pnk while} \\ \colorbox{pink}{text that I thought addresses
this question is highlited in pink}}
\item{\textcolor{teal}{Remarks on the pros and cons of MGD approaches with respect to fluid flow driven transients are 
suggested in teal while} \\ \colorbox{teal}{text that I thought addresses
this question is highlited in teal}}
\item{\textcolor{magenta}{Remarks on the effects of flow direction effects are 
suggested in magenta while}\\  \colorbox{magenta}{text that I thought addresses
this question is highlited in magenta}}
\end{enumerate}
...
\par The history of MSRs began with the Aircraft Reactor Experiment at Oak Ridge National
 ...
 equivalent full-power hours with both \ce{^{233}U} and \ce{^{235}U} fuels at different
...
flow on various quantities of interest related to CFRs. 
Figure \ref{fig:zhang_2d_flux} from \cite{zhang_development_2009-1} shows that the
...
\ref{tbl:dulla_flow_regimes_beta} shows, regardless of the flow regime chosen,
...
velocity, leading to subsequent Doppler reactivity feedback with this effect
having a greater reactivity contribution than the increased loss of DNPs.
Figure \ref{fig:zhang_residence_time_keff} illustrates 
 the expected reduction of
 $k_{eff}$ with increasing out-of-core residence time while inflow velocity
 and inlet temperature are held constant.
...
challenges to traditional methods for reactor kinetics modeling, \colorbox{pink}{particularly
point-kinetics approaches} \\ \colorbox{pink}{ which assume a constant value for $\beta_{eff}$.} In
...
\colorbox{orange}{In terms of fidelity, within the modelling approaches considered for this
work,} \\ \colorbox{orange}{ multi-group diffusion (MGD) approaches are the gold standard because they
make} \\ \colorbox{orange}{ the fewest assumptions and simplifications out of any of the models.}
\colorbox{blue}{This increases their computational cost, however, making these methods} \\ \colorbox{blue}{
the most computationally expensive of all the approaches considered, at least}
\\ \colorbox{blue}{those with computational cost data available.} However the multi-group
...
\colorbox{yellow}{The results of the} \\ \colorbox{yellow}{ DYN3D-MSR model of the MSRE pump start-up transient,
along with the} \\ \colorbox{yellow}{ experimental values and those obtained using a similar 1D code,
DYN1D-MSR, are presented in Figure \ref{fig:krepel_dyn3d_msre_pump_start}.} \colorbox{teal}{The most
immediate takeaway is that when using the JEFF2.2 libraries, both} \\ \colorbox{teal}{ DYN3D-MSR and
DYN1D-MSR are able to closely match the experimental values,} \\ \colorbox{teal}{though they appear to
miss the reactivity oscillations later in the transient.} \textcolor{teal}{The
teal highlight isn't the part showing that MGD is best for flow induced
transients, raather, this whole section address both of those questions. It may
also be beneficial to again point out that the missed osciliation alter in the
transient are likely due to the speed lmited control rods though Krepel does
not expelicitly state this.} \textcolor{green}{It may also be beneficial to
note here} 
...
\colorbox{pink}{ This paragraph.} While these assumptions would not seem to connect well
...
of the transient. \textcolor{pink}{This statement would seem to contradict my claim that PRK is not suited for flow transients. But it may be countered by
this being at power.} \textcolor{green}{This is a good place to point out
that DNPs are less important in "at power" situations.} In
...
\cite{shi_development_2016} is seen in Figure \ref{fig:shi_msre_ps}. \colorbox{green}{\textcolor{pink}{This is a good demonstration of the shortcomings of PRK approaches and that they}} \\ \colorbox{green}{\textcolor{pink}{ capture average behavior decently.}}
...
Two transients in the MSFR fast system, each changing the extracted power on the secondary side by
...
in Figure \ref{fig:lucotte_ipk}. No work using the IPK method was found which
analyzed a thermal system. \textcolor{pink}{Shows how prk misses osciliations.}
...
\textcolor{teal}{Shows perforamnce of MGD in flow induced transients.}\textcolor{pink}{Shows underperformance of PRK in flow induced transients.}\textcolor{yellow}{Suggest MGD for flow induced transients.} \textcolor{green}{Shows relative miportance of DNPs to flow induced transients while the ULOFs in MSRs show that
this is swamped by thermal effects.}
In this paper
...
reach a compromise between MGD and the various implementations of PRK. \colorbox{blue}{ This section} While
...
\par \colorbox{blue}{The time at which these updates occur is key to the computational
 cost savings of
...
updates, or at least in the presence of the perturbation. \colorbox{blue}{Without an adaptive
updating scheme, the requirement of frequent} \\ \colorbox{blue}{ shape updates through an MGD
calculation can strongly reduce the computational savings} \\ \colorbox{blue}{ offered by
quasi-statics.}
...
Equation \ref{eq:spk_move}. Equation \ref{eq:spk_uni} links
...
\label{sec:comp}
\colorbox{orange}{ This entire paragraph} In terms of suitability for the kinetics analyses of CFRs there should be no
...
\colorbox{blue}{The following sentence.}{It is evident that while quasi-statics can save computational cost with some
...
\textcolor{green}{Could discuss dnp's low importance at power here.}
...
\par \colorbox{pink}{ This entire section.} DPK methods may effectivley capture transient evolutions at power and
 with a loss
...
\colorbox{teal}{Following sentence.}{In the likely event that even the most rigorous PRK approaches fail to capture
...
\colorbox{magenta}{This entire section.}
