...
that do not allow for moving prcursors, equations \ref{eq:prk} and \ref{eq:prk_dnp}
.
...
\end{equation}
 ...
 Neither of these assumptions connect well
with the physics of CFRs. In response to this disconnect several methods have been
proposed to adjust the PRK equations or its implementation. In the following
sections these methods will be elaborated on; while differing researchers
...
\subsubsection{delayed point reactor kinetics} \label{sssec:dpk}
What are termed as
...
\end{equation}
  ...
  the kinetics of CFRs, several methods have been developed to adjust PRK for use
in CFR kinetics analysis. \ldots However, in figure \ref{fig:zhang_comparison_pk} a
direct comparison between two modified PRK schemes, MPM represeting 

\begin{figure}[h]
   \centering
   \includegraphics[width=0.8\textwidth]{Zhang_Comparison_2009_Fig_3}
   \caption{Figure 9 from \cite{zhang_comparison_2009}.} 
   \label{fig:zhang_comparison_pk}
\end{figure}
